  ...
  try {
    ...
    for (let i = 0; i < incomes.length; i++) {
      ...
      switch (incomes[i]?.recurring) {
        case 'One-time':
          if (
            incomes[i]?.date?.getMonth() === new Date().getMonth() &&
            incomes[i]?.date?.getFullYear() === new Date().getFullYear()
          ) {
    ...
    for (let i = 0; i < expenses.length; i++) {
      ...
      switch (expenses[i]?.recurring) {
        case 'One-time':
          if (
            expenses[i]?.date?.getMonth() === new Date().getMonth() &&
            expenses[i]?.date?.getFullYear() === new Date().getFullYear()
          ) {
        ...
        case 'Bi-Weekly':
          amount = parseFloat(
            (((expenses[i]?.amount || 0) * 26) / 12).toFixed(2)
          );
          break;
        case 'Semi-Monthly':
    ...
    for (let i = 0; i < mortgages.length; i++) {
      ...
      switch (mortgages[i]?.frequency) {
        case 'Bi-Weekly (every 2 weeks)':
          amount = parseFloat(
            (((mortgages[i]?.payments.payment || 0) * 26) / 12).toFixed(2)
          );
          break;
        case 'Semi-Monthly (24x per year)':
          amount = parseFloat(
            ((mortgages[i]?.payments.payment || 0) * 2).toFixed(2)
          );
          break;
    ...
    if (flexibleIncome <= 0) {
      ...
      recommendedBudget = {
        ...
        transportation: parseFloat((flexibleIncome * 0.1).toFixed(2)),
